...
export default {
  ...
  watch: {
    prefill () {
      if (this.prefill) {
        if (this.prefill instanceof File || typeof this.prefill === 'string') {
          this.preloadImage(this.prefill)
        } else {
          console.warn('The prefill property must be string or an instance of a File object.')
        }
      } else {
        this.removeImage()
      }
